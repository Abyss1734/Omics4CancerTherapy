...
# Загрузка данных
df <- read_csv("./cervical_cancer_sik3_bosutinib_data.csv")
...
plot_title <- glue("{drug_name} AUC vs {gene_name} Depscore for {cancer_type}")  # Используем оригинальное название
title_width <- nchar(plot_title)
title_size <- ifelse(title_width > 50, 18, 
                     ifelse(title_width > 30, 20, 24))

# Расчёт корреляции
corr <- cor.test(df$Depscore, df$AUC, method = "spearman")
...
file_name <- glue("{drug_name}_AUC_vs_{gene_name}_Depscore_for_{str_replace_all(cancer_type_filename, ' ', '_')}.png") %>% 
  str_to_lower()
