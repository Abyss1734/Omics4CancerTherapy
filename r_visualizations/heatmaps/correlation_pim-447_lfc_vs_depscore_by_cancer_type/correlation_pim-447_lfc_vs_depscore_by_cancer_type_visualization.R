...
correlation_file <- "./correlation_results_pim-447_lfc_vs_depscore_by_cancer_type.csv"
p_value_file <- "./p_value_results_pim-447_lfc_vs_depscore_by_cancer_type.csv"
...
png(file = paste0(output_dir, "correlation_heatmap_pim-447_lfc_vs_depscore_by_cancer_type.png"), width = 10, height = 10, units = "in", res = 600)
draw(ht_correlation)
dev.off()

# Сохранение тепловой карты для p-значений
png(file = paste0(output_dir, "p_value_heatmap_pim-447_lfc_vs_depscore_by_cancer_type.png"), width = 10, height = 10, units = "in", res = 600)
draw(ht_p_value)
dev.off()
